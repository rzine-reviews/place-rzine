...
This document relies on the following packages: 
  ...
  * [tidyverse](https://www.tidyverse.org/): The tidyverse R package is a collection of packages that provides a cohesive framework for data manipulation, visualization, and analysis using a consistent and intuitive syntax.
...
The example data used in this tutorial was created by the author from a directory of the American University Club of Shanghai published in 1936 [@shanghai_american_1936], [@armand_bonding_2024]. The original dataset can be downloaded from [Zenodo](https://zenodo.org/record/8047064). It is freely accessible and open for reuse. In this tutorial, we shall use a simplified version of the original dataset, which we describe below. 
...
*kable* is a function from the *knitr* package used for creating tables in a nicely formatted way. *kable_styling* is a function that sets the styling options for the table: 
  ...
  * position = "left" specifies the position of the table on the page, in this case, aligning it to the left.</div> 
...
<div class="alert alert-success" role="alert">  
This part is not directly reproducible because it depends on the intrinsic qualities of the dataset. Nevertheless, it is worth mentioning because the rationale can be adapted to other data and research questions. The places were coded manually based on the attributes of the universities and the students they contained. For students, the attributes used were nationality, period of study, and the nature of their degree. For universities, the attribute used was the region in which they were located (East Coast, West, or Midwest).</div>

While it is beyond the scope of this tutorial to detail the coding methodology used in this study, we provide the code below for extracting students and universities from each place. 

First, we extract individual elements (students) and sets (universities) from the PlaceDetail column, using "-" as the separator and we remove special characters (-, {, }) for cleaner extraction.
```{r}

split1 <- Result1_df %>% mutate(PlaceDetail,
                               Student=str_extract(PlaceDetail,"[^\\}]+\\}"),
                               University=str_extract(PlaceDetail,"\\} - \\{.*")) %>% 
  mutate(Student = str_remove_all(Student,"\\}")) %>% 
  mutate(Student = str_remove_all(Student,"\\{")) %>% 
  mutate(University = str_remove_all(University,"\\} - \\{")) %>% 
  mutate(University = str_remove_all(University,"\\}")) 
...
```
<br>
Next, we create one row per student by splitting places with multiple students separated by ";". We use the "separate_rows" function from the tidyverse package: 
```{r}
split2 <- split1 %>% separate_rows(Student, sep=";")
...
<br>
Similarly, we create one row per university by splitting places with multiple universities separated by ";": 
```{r}
split3 <- split2 %>% separate_rows(University, sep=";") 

kable(head(split3)) %>%
  kable_styling(bootstrap_options = "striped", full_width = T, position = "left")
```
<br>
This code can be reused to extract elements and sets from places in order to manually label places for other specific studies. 
...
To build a network of places linked by universities, we begin by creating an adjacency matrix^[__Adjacency Matrix__: an adjacency matrix is a square matrix used to represent a finite graph. The elements of the matrix indicate whether pairs of vertices are adjacent or not in the graph.] from the edgelist: 
...
V(Net2)$size <- Result1_df$NbElements
...
plot(Net2, vertex.size = Result1_df$NbElements, 
     vertex.color = "light blue", 
     vertex.label = NA, 
     main="Network of universities linked by places")
...
```
<br>
In both cases, the summary indicates that the two networks are undirected (U), named (N), weighted (W) networks (UNW), which are the options chosen during the transformation. The network of places linked by universities (Net1) contains 223 vertices (places) and 1606 ties (universities). The network of universities linked by places includes 146 vertices (universities) and 197 ties (places). The name of vertices is the only attribute.   
...
```
<br>
Graph density is useful mostly when comparing networks of similar order (with similar number of vertices). In our example, the network of places linked by universities is denser than the network of universities linked by places.
...
There are many metrics to measure the relative position of vertices in networks. In the following, we focus on the most popular centrality metrics, which can all be computed with the *igraph* package: 
  ...
  * **Degree**: the number of ties a node has. It is the simplest measure of centrality. In the following, we use a normalized version of the measure in order enable comparisons across networks built from different data structure. Since the projected networks are valued, it is also possible to calculate a weighted degree based on the intensity of the links. In the network of universities linked by places, for example, it measures the number of connections or ties each university has with other universities in the network. It can be interpreted as a measure of university popularity or the extent to which a university is actively connected to other institutions.
  * **Eigenvector**: the number of connections a node has to other well-connected nodes. It is a measure of the influence of a node in a network. In the network of universities, it quantifies the extent to which a university is connected to other influential universities in the network. Universities with high eigenvector centrality are connected to other highly influential institutions, indicating their own influence and prestige.
  * **Betweenness**: the number of times a node acts as a bridge along the shortest path between two other nodes. In this sense, the more central a node is, the greater control it has over the flows that goes through it. It is often considered as a measure of brokerage, or the capacity of a node to mediate between other nodes. Betweenness centrality is a common metrics, but in the context of projecting a bimodal network, its interpretation becomes more challenging. In the network of universities linked by places, it quantifies the university’s brokering power or the frequency with which it falls on the shortest path between other universities. Universities with high betweenness centrality play a crucial role in connecting different parts of the network and facilitating communication and collaboration between institutions. 
...
Net2MC <- induced.subgraph(Net2,vids=clusters(Net2)$membership==1)
```
<br>
...
The *igraph* package offers [various methods](https://igraph.org/r/doc/communities.html) for detecting communities. In this document, we selected the Louvain algorithm [@blondel_fast_2008], one of the most popular methods for finding communities, especially but not exclusively in large networks.   
